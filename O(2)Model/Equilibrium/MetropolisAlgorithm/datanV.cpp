...
double beta, E, Cv, M, chi, rho, t, vort, avort; 
double dE, dCv, dM, dchi, drho, dvort, davort;
...
int main(){
    ...
    std::ofstream Mfile; std::ofstream Vortfile; std::ofstream Avortfile;
    ...
    for(int i = 0; i<cont-1; i++){
        ...
        //Cluster algorithm
            ...
            if (v == 0){
                ...
                Ntherm = (int) DATA[0]; Nmeas = (int) DATA[1]; Nstep = (int) DATA[2]; L = (int) DATA[3];
                beta = DATA[4]; E = DATA[5]; dE = DATA[6]; vort = DATA[7]; dvort = DATA[8]; 
                avort = DATA[9]; davort = DATA[10]; t = DATA[11]; 
                char NameVort[100], Vort_str[100], NameE[100], E_str[100], NameTime[100], Time_str[100];
                char NameAvort[100], Avort_str[100];

                if (i == 0){
                    sprintf(NameE, "%dDXY_E_L%d_Meas%d.txt", d,L, Nmeas);
                    sprintf(NameVort, "%dDXY_Vort_L%d_Meas%d.txt", d, L, Nmeas);
                    sprintf(NameAvort, "%dDXY_Avort_L%d_Meas%d.txt", d, L, Nmeas);
                    sprintf(NameTime, "%dDXY_Time_L%d_Meas%d.txt", d, L, Nmeas);
                    Vortfile.open(NameVort); Avortfile.open(NameAvort); Efile.open(NameE); Timefile.open(NameTime);
                }
                sprintf(E_str, "%-30.17g%-30.17g%-30.17g\n", beta, E, dE);
    ...
    if(alg == 1 && vort == 0){Mfile.close(); Cvfile.close(); Chifile.close();}
    if(alg == 1 && vort == 1){Vortfile.close(); Avortfile.close();}
