...
inline void EnergyWA(double beta, std::vector<double> Inu, std::vector<double> Inu1, std::vector<double> Inu2){
    ...
    for(int i = 0; i < L; i++){
        for(int j = 0; j < L; j++){
            for(int k = 0; k < L; k++){
                Tab = BesselTableCaller(beta, FluxX[i][j][k], Inu, Inu1, Inu2);
                Iv = Tab.Inu; Iv1 = Tab.Inu1; Iv2 = Tab.Inu2;
                En -= Iv1/Iv; En2 += Iv2/Iv - (Iv1/Iv)*(Iv1/Iv);
                Tab = BesselTableCaller(beta, FluxY[i][j][k], Inu, Inu1, Inu2);
                Iv = Tab.Inu; Iv1 = Tab.Inu1; Iv2 = Tab.Inu2;
                En -= Iv1/Iv; En2 += Iv2/Iv - (Iv1/Iv)*(Iv1/Iv);
                Tab = BesselTableCaller(beta, FluxZ[i][j][k], Inu, Inu1, Inu2);
                Iv = Tab.Inu; Iv1 = Tab.Inu1; Iv2 = Tab.Inu2;
                En -= Iv1/Iv; En2 += Iv2/Iv - (Iv1/Iv)*(Iv1/Iv);
            }
        }
...
inline void AccRatio(int mu, double beta, std::vector<double> Inu, std::vector<double> Inu1, std::vector<double> Inu2){
    ...
    else if (mu == 2){FluxIni = -FluxY[modulo(Masha[0]-1,L)][Masha[1]][Masha[2]];}
    else if (mu == 3){FluxIni = FluxY[Masha[0]][Masha[1]][Masha[2]];}
    else if (mu == 4){FluxIni = -FluxZ[Masha[0]][Masha[1]][modulo(Masha[2]-1,L)];}
    else if (mu == 5){FluxIni = FluxZ[Masha[0]][Masha[1]][Masha[2]];}
    FluxFin = FluxIni + 1;
...
// mu = 4 --> z direction, mu = 5 --> -z direction //
    ...
    if (r < R){
        if (mu == 0){
            ...
            FluxX[Masha[0]][modulo(Masha[1]-1,L)][Masha[2]] -= 1;
            Masha[1] = modulo(Masha[1]-1,L);
        }
        else if (mu == 2){
            FluxY[modulo(Masha[0]-1,L)][Masha[1]][Masha[2]] -= 1;
            Masha[0] = modulo(Masha[0]-1,L);
        }
        else if (mu == 3){
            FluxY[Masha[0]][Masha[1]][Masha[2]] += 1;
            Masha[0] = modulo(Masha[0]+1,L);
        }
        else if (mu == 4){
            FluxZ[Masha[0]][Masha[1]][modulo(Masha[2]-1,L)] -= 1;
            Masha[2] = modulo(Masha[2]-1,L);
        }
        else if (mu == 5){
            FluxZ[Masha[0]][Masha[1]][Masha[2]] +=1;
            Masha[2] = modulo(Masha[2]+1,L);
        }
    }
}
